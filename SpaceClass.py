...
class Space():
    ...
    def propertyAction(self, activePlayer, board, roll):
        isDisplayed = True
        imgMod = ImageModifier()
        font = pygame.font.SysFont(None, 20)
        smallFont = pygame.font.SysFont(None, 16)
        if ((self.owner == None) & (activePlayer.money >= self.buyCost)):
            messageText = font.render("Would you like to buy " + self.spaceName + "?", True, self.BLACK)
            ...
            if ((self.owner == activePlayer) & (self.numHouses < 5) & (activePlayer.money >= self.houseCost) & (self.spaceName not in ["Marth", "Ike", "Roy", "Byleth", "Pac Man", "Mr Game & Watch"])):
                messageText = font.render("Would you like to buy a house?", True, self.BLACK)
                textRect = messageText.get_rect()
                textRect.center = (500, 700)
                buttonText = smallFont.render("No", True, self.BLACK)
                buttonRect = buttonText.get_rect()
                ...
                if ((self.spaceName == "Pac Man") or (self.spaceName == "Mr Game & Watch")):
                    if ((self.owner.calcNumUtils() == 2)):
                        rent = 10 * roll
                        activePlayer.money -= (10 * roll)
                        self.owner.money += (10 * roll)
                    else:
                        rent = 4*roll
                        activePlayer.money -= (4 * roll)
                        self.owner.money += (4 * roll)
    ...
    def setHouses(self, board, rect):
        if self.spaceName in ["Ganondorf", "Little Mac", "King Dedede", "Kirby", "Meta Knight", "Falco", "Wolf", "Fox", "Samus", "Dark Samus", "Zero Suit Samus"]:
            houseWidth = rect[2]/4
            if self.numHouses == 5:
                board.blit(ImageModifier.space_modifier('hotel.jpg', houseWidth, rect[3], 0), (rect[0], rect[1]))
            else:
                for i in range(self.numHouses):
                    board.blit(ImageModifier.space_modifier('house.jpg', houseWidth, rect[3], 0), (rect[0] + houseWidth * i, rect[1]))
            ...
            if self.numHouses == 5:
                board.blit(ImageModifier.space_modifier('hotel.jpg', rect[2], rect[3], 0), (rect[0], rect[1]))
            else:
                for i in range(self.numHouses):
                    board.blit(ImageModifier.space_modifier('house.jpg', rect[2], rect[3], 0), (rect[0], rect[1] + houseHeight * i))
